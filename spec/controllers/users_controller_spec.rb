  ...
  context 'Signed in as an admin' do
    ...
    context "POST /users" do
      context "when the create is successful" do
        let(:create_params) { {user: Fabricate.attributes_for(:user)} }
      ...
      context "when the create is unsuccessful" do
        let(:user) { Struct.new(:admin, :attributes).new(true, {}) }
    ...
    describe "#user_params" do
      context "with current user not admin" do
        ...
        it 'not have admin field' do
          expect(controller.send(:user_params).keys).to eq(%w{name})
        end
